...
public class InMemoryProductRepository implements IproductRepository{
	...
	public Set<Product> getProductsByPriceFilter(
			Map<String, List<String>> filterParams) {
		Set<Product> productsByHigherPrice = new HashSet<Product>();
	    Set<Product> productsByLowerPrice = new HashSet<Product>();
	    ...
	    if(criterias.contains("high")) {
		      for(String price: filterParams.get("high")) {
		    	  ...
		    	  for(Product product: listOfProducts) {
		    		  if(product.getUnitPrice().compareTo(p) == -1){
		    			  productsByLowerPrice.add(product);
		          }
		        }
		      }
		 }
	    if(criterias.contains("low")) {
		      for(String price: filterParams.get("low")) {
		    	  ...
		    	  for(Product product: listOfProducts) {
		    		  if(product.getUnitPrice().compareTo(p) == 1){
		    			  productsByHigherPrice.add(product);
		          }
		        }
		      }
		 }
	    productsByLowerPrice.retainAll(productsByHigherPrice);
